...
- **`cluster-sim.html` — 调度策略仿真：命中率会收敛吗**
  ...
  （朴素阈值敏感度、单机容量、索引 TTL）。
  第二节是架构变体——把 400 台机器的容量合并成一个共享 LRU 池，命中率从此跟路由脱钩，
  调度问题缩小成纯负载均衡：对比亲和 / 随机 / 最短队列三种路由，命中率曲线几乎重合，
  但总 TPM、有效 TPM、最长排队积压三张图能看出亲和仍有真实吞吐损失，随机路由在拥堵下
  会排队失稳；这里的反压是真实的——一轮没处理完，下一轮不会发生，损失体现在吞吐和排队里，
  不体现在命中率上。
